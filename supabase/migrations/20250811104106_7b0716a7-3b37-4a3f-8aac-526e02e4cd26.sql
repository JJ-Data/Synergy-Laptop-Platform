-- Invitations feature: table, RLS, and helper functions
...
create policy if not exists "Super admins manage invitations"
...
-- Admins can manage employee invitations for their own company
create policy if not exists "Admins manage employee invites in company"
  on public.invitations
  as permissive
  for all
  using (
...
-- Optional: allow reading by token via function only (no direct select needed)
